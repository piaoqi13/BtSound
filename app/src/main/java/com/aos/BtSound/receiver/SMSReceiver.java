...
public class SMSReceiver extends ContentObserver {
    public static final String SMS_URI_ALL = "content://sms/";              // 所有的短信
    public static final String SMS_URI_INBOX = "content://sms/inbox";       // 收件箱短信
    public static final String SMS_URI_SEND = "content://sms/sent";         // 发件箱短信
    public static final String SMS_URI_DRAFT = "content://sms/draft";       // 草稿箱短信
    private Context mContext = null;                                        // Context上下文
    ...
    private Activity mActivity = null;                                      // Act上下文
    private Handler mHandler = null;                                        // Handler对象
    private List<SmsInfo> mSmsInfos = null;                                 // 装载短信集合
        ...
        if (VoiceCellApplication.isReadSMS) {
            ...
            if (!mSmsInfos.isEmpty()) {
                for (int i = 0; i < VoiceCellApplication.mContacts.size(); i++) {
                    if (mSmsInfos.get(0).getName().equals(VoiceCellApplication.mContacts.get(i).getName())) {
                        setParam();
