...
public abstract class BluetoothHeadsetUtils
{
    ...
    public boolean start()
    {
        ...
        {
            mIsStarted = true;
            DebugLog.d(DebugLog.TAG, "BluetoothHeadsetUtils:start "
    ...
    {
        DebugLog.d(DebugLog.TAG, "BluetoothHeadsetUtils:stopBluetooth "); //$NON-NLS-1$
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver()
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            if (action.equals(BluetoothDevice.ACTION_ACL_CONNECTED))
            {
                ...
                DebugLog.d(DebugLog.TAG, "BluetoothHeadsetUtils:onReceive "
                        + mConnectedHeadset.getName() + " connected"); //$NON-NLS-1$
            }
            else if (action.equals(BluetoothDevice.ACTION_ACL_DISCONNECTED))
            {
                DebugLog.d(DebugLog.TAG, "BluetoothHeadsetUtils:onReceive "
                        + "Headset disconnected"); //$NON-NLS-1$
            ...
            else if (action.equals(AudioManager.ACTION_SCO_AUDIO_STATE_CHANGED))
            {
                ...
                if (state == AudioManager.SCO_AUDIO_STATE_CONNECTED)
                {
                    ...
                    onScoAudioConnected();
                    DebugLog.d(DebugLog.TAG, "BluetoothHeadsetUtils:onReceive "
                            + "Sco connected"); //$NON-NLS-1$
                }
                else if (state == AudioManager.SCO_AUDIO_STATE_DISCONNECTED)
                {
                    DebugLog.d(DebugLog.TAG, "BluetoothHeadsetUtils:onReceive "
                            + "Sco disconnected"); //$NON-NLS-1$
    ...
    private BluetoothProfile.ServiceListener mHeadsetProfileListener = new BluetoothProfile.ServiceListener()
    {
        ...
        public void onServiceConnected(int profile, BluetoothProfile proxy)
        {
            ...
            if (devices.size() > 0)
            {
                ...
                mIsCountDownOn = true;
                mCountDown11.start();
                DebugLog.d(DebugLog.TAG, "BluetoothHeadsetUtils:onServiceConnected "
                        + "Start count down"); //$NON-NLS-1$
            }
    ...
    private BroadcastReceiver mHeadsetBroadcastReceiver = new BroadcastReceiver()
    {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(BluetoothHeadset.ACTION_CONNECTION_STATE_CHANGED)) {
                ...
                DebugLog.d(DebugLog.TAG, "BluetoothHeadsetUtils:onReceive "
                        + "\nAction = " + action + " State = " + state); //$NON-NLS-1$ //$NON-NLS-2$
